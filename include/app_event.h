 ...
 * @remarks If a sender application sends same event via trusted API and non-trusted API, then a trusted receiver should can get latest data regardless of trusted or non-trusted, but non-trusted receiver should can get only the last data from non-trusted API.
