 ...
 *
 * @since_tizen 3.0
 * @remarks The receiver applications will be received this last event data after adding their new handlers via event_add_event_handler API since the sender application called this API.
 * @remarks If a sender application sends same event via trusted api and non-trusted api, then a trusted receiver should can get latest data regaredless of tursted or non-trusted, but non-trusted receiver should can get only a last data from non-trusted api.
