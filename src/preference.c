...
static int _prepare_and_bind_stmt(char *buf, const char *text1,
		const char *text2, const char *text3, sqlite3_stmt **stmt)
{
	int ret;
	...
	ret = sqlite3_bind_text(*stmt, 1, text1, -1, SQLITE_STATIC);
	if (ret != SQLITE_OK) {
	...
	ret = sqlite3_bind_text(*stmt, 2, text2, -1, SQLITE_STATIC);
	if (ret != SQLITE_OK) {
	...
	ret = sqlite3_bind_text(*stmt, 3, text3, -1, SQLITE_STATIC);
	if (ret != SQLITE_OK) {
...
static int _write_data(const char *key, const char *type, const char *data)
{
	...
	if (exist)
		...
		ret = _prepare_and_bind_stmt(buf, key, type, data, &stmt);
